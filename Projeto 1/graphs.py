...
def main():
    ...
    plt.plot(nOwnersList, MPackStdRList, label="JSON")
    plt.plot(nOwnersList, MPackStdWList, label="MessagePack")
    plt.xlabel("Número de Owners")
    plt.ylabel("Tempo (s)")
    plt.title("Tempo de leitura por nº de Owners (max 10 pets p/ owner)")
    plt.legend()
    plt.show()
